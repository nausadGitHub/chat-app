...
@Component({
        ...
        <ion-content class="ion-no-padding">
                <ion-router-outlet id="auth" [environmentInjector]="environmentInjector"></ion-router-outlet>
        </ion-content>
        `,
        styles: [
                `.header-auth {
...
export default class AuthRootPageComponent {
        ...
        changeStatus = (url: string) => {
                this.ngzone.run(() => {
                        if (Capacitor.getPlatform() !== 'web') StatusBar.setBackgroundColor({ color: url?.startsWith('/user') ? '#1C2D35' : '#89A619' });
                });
        }
